...
Brief: `../vetload-platform/docs/components/C04-corpus.md` (private). Design: [ARCHITECTURE.md](ARCHITECTURE.md). Decisions: [ADR-0049 Wave 1 alignment](https://github.com/Vetload/vetload-platform/blob/main/docs/architecture/decisions/0049-wave-1-alignment.md) (program, overrides anything here), [ADR-0001](docs/architecture/decisions/0001-generator-runtime-and-container.md), [ADR-0002](docs/architecture/decisions/0002-expected-result-versioning.md), [ADR-0003](docs/architecture/decisions/0003-release-format-and-consumption.md). First files: [seed corpus](docs/seed-corpus.md).
...
| M2 Seed release `v0.1.0` | Wave 1, **gate G1** | After `contracts/v0.1.0` exists, 167 files (the target was about 150) are released with checksums and attestations, and the G1 proof workflow is green. C06 loading it is a tracker follow-up (ADR-0049 G1) |
...
2. `container/Dockerfile`: `debian:trixie-slim` by digest, apt from `snapshot.debian.org` at a fixed timestamp, exact versions in `container/apt-packages.txt`, and `uv` copied by digest. An `image.yml` workflow publishes `ghcr.io/vetload/corpus-generator`. Release checklist: after the first publish, the founder makes the package public in its settings, because GitHub has no API for it (ADR-0049 F5).
...
7. Once `contracts/v0.1.0` is tagged, vendor `vetload-corpus-schema-0.1.0.tar.gz` into `schema/vendor/` with `contracts.lock` (ADR-0003), and set `contracts_version` and `contracts_schema`. `tools/verify.py` checks each reason against its outcome, each kind and each risk flag against the vendored registries. Tag `v0.1.0` only after this (ADR-0049 E4).
...
| Schema and registry validation | A reason on `success`, no reason on `corrupt_media`, `kind: unknown`, `facts` inside `expected`, or a missing licence fails (demonstrated on the draft in this pull request). A reason from another outcome or an unknown flag fails the registry check | Validation reports how many entries it checked |
| Vendored contracts | Change one byte of a vendored file; the `contracts.lock` check fails | Reports the files checked |
...
| Action | Blocks |
| --- | --- |
| Allow `Vetload/vetload-corpus` to publish packages to GHCR under the `vetload` organisation, then make `corpus-generator` public after its first publish (ADR-0049 F5) | M1 image publishing |
